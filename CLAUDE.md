...
`database.py` opens SQLite with `journal_mode=WAL`, `busy_timeout=5000`, `synchronous=NORMAL` and registers a Python-based `lower()` UDF so case-insensitive search via `func.lower()` works on Cyrillic and other non-ASCII text (SQLite's built-in only folds ASCII A–Z). Don't replace it.

## Running the webapp

Both dev and prod machines run the backend via the systemd unit `urantia-library.service`, which executes `webapp/start.sh` as user `tigran` with `WorkingDirectory=/Books/` and `EnvironmentFile=/Books/urantia-library/webapp/secrets.env`. `start.sh` `exec`s uvicorn on `127.0.0.1:8000` with `--no-access-log --root-path "${APP_ROOT_PATH:-/}"`, and adds `--reload` **only when `APP_ENV=development`**. The SPA mount point is controlled by `APP_ROOT_PATH`: prod sets it to `/library` (matching nginx, which still serves the app under `/library/` and strips that prefix before forwarding to uvicorn), dev leaves it unset so both default to `/`. `vite.config.ts` reads the same env var as its `base` so built asset URLs line up with the uvicorn root-path.
