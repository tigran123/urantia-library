  ...
  >
    ...
    <div class="flex flex-row flex-grow min-h-0 overflow-hidden" :class="immersive ? '' : 'rounded-b-lg'">
      ...
      <div
        ...
        class="html-scroll relative flex-grow min-w-0 overflow-y-auto bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
      >
        <div
          v-if="!rawMode"
          ref="contentEl"
          class="html-content w-full max-w-5xl mx-auto px-4 sm:px-5 lg:px-6 py-8 leading-relaxed"
        ...
        <pre
          v-else
          class="html-raw w-full max-w-5xl mx-auto px-4 sm:px-5 lg:px-6 py-8 whitespace-pre-wrap break-words"
          :style="{ fontSize: `calc(${fontScale} * var(--reader-base, 16px))`, fontFamily }"
        >{{ raw }}</pre>
        <AnnotationPopover
