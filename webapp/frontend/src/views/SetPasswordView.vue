...
<template>
  <div :key="locale" class="max-w-md mx-auto mt-10 bg-white dark:bg-gray-800 p-8 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
    <h2 class="text-2xl font-bold mb-6 text-center text-gray-900 dark:text-white">{{ t('auth.setPasswordTitle') }}</h2>
    ...
    <form v-if="!successMsg && token" @submit.prevent="handleSetPassword" class="space-y-4">
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ t('auth.setPasswordFullNameLabel') }} <span class="text-gray-400 dark:text-gray-500 font-normal">{{ t('auth.setPasswordOptional') }}</span></label>
        <input
          v-model="realName"
          type="text"
          maxlength="100"
        ...
        />
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ t('auth.setPasswordFullNameHelp') }}</p>
      </div>
